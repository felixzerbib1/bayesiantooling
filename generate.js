...
function renderPill(value, flag, applicable) {
...
function renderMatrixView() {
  const pairs = getCustomerProductPairs().filter(p => {
    if (state.customerFilter !== "all" && p.customer !== state.customerFilter) return false;
  ...
  if (pairs.length === 0) {
    document.getElementById("matrixView").innerHTML = \`<div class="no-results"><span class="no-results-icon">&#128269;</span><div class="no-results-text">No configurations match filters</div><div class="no-results-sub">Try adjusting your filters</div></div>\`;
    return;
  }

  let html = '<div class="matrix-container"><table class="matrix-table"><thead><tr><th>Feature Flag</th>';
  for (const p of pairs) {
    html += \`<th><div class="col-header-customer">\${p.customer}</div><div class="col-header-product">\${p.product}</div></th>\`;
  }
  html += '</tr></thead><tbody>';
  ...
  for (const cat of categories) {
    ...
    const visibleFlags = catFlags.filter(f => {
      ...
      if (state.statusFilter !== "all") {
        const anyMatch = pairs.some(p => {
          if (!isFlagApplicable(f, p.product)) return false;
          const val = getFlagValue(p.customer, p.product, f.key);
    ...
    hasVisibleRows = true;
    html += \`<tr class="category-row"><td colspan="\${pairs.length + 1}">\${cat.label}</td></tr>\`;
    for (const flag of visibleFlags) {
      html += \`<tr><td>\${flag.name}</td>\`;
      for (const p of pairs) {
        const applicable = isFlagApplicable(flag, p.product);
        const val = getFlagValue(p.customer, p.product, flag.key);
        const note = getFlagNote(p.customer, p.product, flag.key);
        html += \`<td>\${renderPill(val, flag, applicable)}\${note ? '<span class="note-text">' + note + '</span>' : ''}</td>\`;
      }
      html += '</tr>';
    }
  }
