...
function renderMatrixView() {
  ...
  for (const product of products) {
    ...
    for (const cat of categories) {
      ...
      productRowsHtml += \`<tr class="category-row"><td colspan="\${customers.length + 1}">\${cat.label}</td></tr>\`;

      for (const flag of visibleFlags) {
        productRowsHtml += \`<tr><td style="font-weight:500">\${flag.name}</td>\`;
        for (const customer of customers) {
    ...
    if (productHasFlags) {
      ...
      html += \`<tr class="product-group-row"><td colspan="\${customers.length + 1}"><span class="product-badge \${productBadgeClass(product)}">\${product}</span></td></tr>\`;
